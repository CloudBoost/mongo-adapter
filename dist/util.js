...
var connector = require('./connector'); // Private Functions
...
function _dropIndex(appId, collectionName, indexString) {
  ...
  try {
    if (indexString && indexString !== '') {
      var collection = connector.client.db(appId).collection(collectionName);
...
mongoService.app = {
  ...
  create: function create(appId) {
    ...
    try {
      ...
      var replSet = connector.replSet();
      var db = new Db(appId, replSet, {
        w: 1
      });
...
mongoService.document = {
  getSearchableDocuments: function getSearchableDocuments(appId, collectionName) {
    var _self = mongoService;
    var deferred = q.defer();
    ...
    try {
      ...
      var collection = connector.client.db(appId).collection(_self.collection.getId(appId, collectionName));
...
mongoService.database = {
  dropDatabase: function dropDatabase(appId) {
    var deferred = q.defer();
    ...
    try {
      ...
      var database = connector.client.db(appId);
...
mongoService.collection = {
  addColumn: function addColumn(appId, collectionName, column) {
    var deferred = q.defer();
  ...
  create: function create(appId, collectionName, schema) {
    var deferred = q.defer();
  ...
  createIndex: function createIndex(appId, collectionName, columnName, columnType) {
    var deferred = q.defer();
    ...
    try {
      ...
      if (Object.keys(obj).length > 0) {
        var collection = connector.client.db(appId).collection(mongoService.collection.getId(appId, collectionName));
  ...
  deleteAndCreateTextIndexes: function deleteAndCreateTextIndexes(appId, collectionName) {
    var deferred = q.defer();
  ...
  getIndexes: function getIndexes(appId, collectionName) {
    var deferred = q.defer();
  ...
  renameColumn: function renameColumn(appId, collectionName, oldColumnName, newColumnName) {
    var deferred = q.defer();
  ...
  dropColumn: function dropColumn(appId, collectionName, columnName, columnType) {
    var deferred = q.defer();
    ...
    try {
      ...
      promises.push(_unsetColumn(appId, collectionName, query)); // Promise List
  ...
  dropCollection: function dropCollection(appId, collectionName) {
    var deferred = q.defer();
  ...
  renameCollection: function renameCollection(appId, oldCollectionName, newCollectionName) {
    var deferred = q.defer();

    try {
      var _self = mongoService;
      var collection = connector.client.db(appId).collection(_self.collection.getId(appId, oldCollectionName));
  ...
  getId: function getId(appId, collectionName) {
    // for a given appId and collectionName it gives a unique collection name
    try {
  ...
  list: function list(appId) {
    var deferred = q.defer();

    try {
      var collection = connector.client.db(appId).collection('_Schema');
      var findQuery = collection.find({});
